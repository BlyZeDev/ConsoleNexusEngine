...
{
    private readonly GlobalController _global;

    internal ConsoleController() => _global = new GlobalController();
    ...
    public bool AddGlobalControl(INexusInputCondition condition, Action action)
        => _global.AddControl(condition, action);
    ...
    /// <returns><see cref="bool"/></returns>
    public bool RemoveGlobalControl(INexusInputCondition condition)
        => _global.RemoveControl(condition);
    ...
    public void GlobalControl(in NexusInputCollection inputs) => _global.Control(inputs);

    private readonly struct GlobalController
    {
        private readonly Dictionary<INexusInputCondition, Action> _controls;

        public GlobalController() => _controls = [];
